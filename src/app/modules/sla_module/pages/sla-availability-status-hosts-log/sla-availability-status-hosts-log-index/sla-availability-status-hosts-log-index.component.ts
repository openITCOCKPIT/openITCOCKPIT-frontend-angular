...
export class SlaAvailabilityStatusHostsLogIndexComponent implements OnInit, OnDestroy, IndexPage {
    private readonly SlaAvailabilityStatusHostsLogService: SlaAvailabilityStatusHostsLogService = inject(
        SlaAvailabilityStatusHostsLogService,
    );
    ...
    public params: SlaAvailabilityStatusLogIndexParams = getDefaultSlaAvailabilityStatusHostsLogIndexParams(
        this.fromParam,
        this.toParam,
    );
    ...
    public load() {
        ...
        this.subscriptions.add(
            this.SlaAvailabilityStatusHostsLogService.getIndex(this.hostId, this.params).subscribe(
                (result: SlaAvailabilityStatusHostsLogIndexRoot) => {
    ...
    private getDeclineValue(index: number): number {
        if (index < this.slaHostAndStatusLog.slaHostStatusLog.length - 1) {
            return (
                this.slaHostAndStatusLog.slaHostStatusLog[index + 1].determined_availability_percent -
                this.slaHostAndStatusLog.slaHostStatusLog[index].determined_availability_percent
            );
        }
        return 0;
    }
