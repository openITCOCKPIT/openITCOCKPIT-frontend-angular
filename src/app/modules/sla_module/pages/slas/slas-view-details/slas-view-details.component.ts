...
export class SlasViewDetailsComponent implements OnInit, OnDestroy {
    ...
    public load() {
        ...
        this.subscriptions.add(
            this.SlasService.getViewDetails(this.slaId, this.params).subscribe(
                (result: SlasViewDetailsRoot) => {
                    ...
                    if (this.details.sla.timeperiod.timeperiod_timeranges) {
                        for (let key in this.details.sla.timeperiod
                            .timeperiod_timeranges) {
                            let timeRange =
                                this.details.sla.timeperiod
                                    .timeperiod_timeranges[key];
                            // Server day 1 - 7
    ...
    private loadContainers() {
        this.subscriptions.add(
            this.ContainersService.loadAllContainers().subscribe((result) => {
