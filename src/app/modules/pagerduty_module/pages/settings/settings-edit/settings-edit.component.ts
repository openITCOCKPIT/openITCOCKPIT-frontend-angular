...
export class SettingsEditComponent implements OnInit, OnDestroy {
    private readonly pagerdutySettingsService: PagerdutySettingsService =
        inject(PagerdutySettingsService);
    ...
    public ngOnInit(): void {
        this.subscriptions.add(
            this.pagerdutySettingsService
                .getPagerdutySettings()
                .subscribe((settings: PagerdutySettings): void => {
                    this.post = settings;
                    this.cdr.markForCheck();
                }),
        );
        const API_KEY_TRANSLATION =
            this.TranslocoService.translate('YOUR_API_KEY_HERE');
    ...
    protected updatePagerdutySettings(): void {
        ...
        this.subscriptions.add(
            this.pagerdutySettingsService
                .setPagerdutySettings(this.post)
                .subscribe((result: GenericResponseWrapper): void => {
                    this.cdr.markForCheck();
                    if (result.success) {
                        this.errors = null;
