    ...
    implements OnInit, OnDestroy
{
    ...
    private renderTrafficlight() {
        ...
        if (this.item() && this.item()?.id) {
            let trafficLightElement = this.document.getElementById(
                'map-trafficlight-' + this.item()?.id,
            );
            if (!this.svgElement) {
                this.svgElement = this.renderer.createElement(
                    'svg',
                    this.svgNamespace,
                );
            }
            ...
            const createLinearGradient = (
                ...
                let grad = this.renderer.createElement(
                    'linearGradient',
                    this.svgNamespace,
                );
            ...
            const redLightPattern = createPattern(
                'redLightPattern_' + this.item()!.id,
                0,
                0,
                3,
                3,
            );
            const redLightCircle = this.renderer.createElement(
                'circle',
                this.svgNamespace,
            );
            ...
            const yellowLightPattern = createPattern(
                'yellowLightPattern_' + this.item()!.id,
                0,
                0,
                3,
                3,
            );
            const yellowLightCircle = this.renderer.createElement(
                'circle',
                this.svgNamespace,
            );
            ...
            const greenLightPattern = createPattern(
                'greenLightPattern_' + this.item()!.id,
                0,
                0,
                3,
                3,
            );
            const greenLightCircle = this.renderer.createElement(
                'circle',
                this.svgNamespace,
            );
            ...
            if (this.item()?.show_label) {
                const text = this.renderer.createElement(
                    'text',
                    this.svgNamespace,
                );
            ...
            const lightsGroup = this.renderer.createElement(
                'g',
                this.svgNamespace,
            );
            this.renderer.setAttribute(
                lightsGroup,
                'id',
                'lights_' + this.item()!.id,
            );

            const redGroup = this.renderer.createElement(
                'g',
                this.svgNamespace,
            );
            this.setAttrs(redGroup, {
                id: `redLightGroup_${this.item()!.id}`,
            });
            const redCircle = this.renderer.createElement(
                'circle',
                this.svgNamespace,
            );
            ...
            const yellowGroup = this.renderer.createElement(
                'g',
                this.svgNamespace,
            );
            this.setAttrs(yellowGroup, {
                id: `yellowLightGroup_${this.item()!.id}`,
            });
            const yellowCircle = this.renderer.createElement(
                'circle',
                this.svgNamespace,
            );
            ...
            const greenGroup = this.renderer.createElement(
                'g',
                this.svgNamespace,
            );
            this.setAttrs(greenGroup, {
                id: `greenLightGroup_${this.item()!.id}`,
            });
            const greenCircle = this.renderer.createElement(
                'circle',
                this.svgNamespace,
            );
            ...
            if (this.showBlue) {
                const blueGroup = this.renderer.createElement(
                    'g',
                    this.svgNamespace,
                );
                this.setAttrs(blueGroup, {
                    id: `blueLightGroup_${this.item()!.id})`,
                });
                const blueCircle = this.renderer.createElement(
                    'circle',
                    this.svgNamespace,
                );
    ...
    private blinking(el: HTMLElement, color: string) {
        ...
        this.timer[color] = setInterval(() => {
            ...
            this.renderer.setStyle(
                el,
                'transition',
                'visibility 0s 1.6s, opacity 1.6s linear',
            );
            setTimeout(() => {
