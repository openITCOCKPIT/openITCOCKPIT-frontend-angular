    ...
    implements OnInit, OnDestroy
{
            ...
            this.MapItemBaseService.getMapItem(params).subscribe(
                (result: MapItemRoot) => {
                    ...
                    if (result.data.Servicestatus) {
                        this.current_state =
                            result.data.Servicestatus.currentState;
    ...
    private renderCylinder(perfdata: any): void {
        ...
        this.createEllipse(
            ...
            rx,
            ry,
            `url(#fadeDarkGray_${this.item()!.id})`,
            0.1,
            2,
            '#CECECE',
            0.5,
            'background_' + this.item()!.id,
        );
        ...
        this.createEllipse(
            ...
            rx,
            ry,
            `url(#fadeDark${stateColor}_${this.item()!.id})`,
            0.8,
        );
        ...
        if (value > 1) {
            this.createRect(
                ...
                rx,
                ry,
                `url(#fade${stateColor}_${this.item()!.id})`,
                0.9,
            );
            //top ellipse
            this.createEllipse(
    ...
    private createLinearGradient(
        ...
        const linearGradient = this.renderer.createElement(
            'linearGradient',
            'svg',
        );
    ...
    private processPerfdata() {
        ...
        if (this.perfdata) {
            this.perfdata.current = parseFloat(
                this.perfdata.current,
            ).toString();
            this.perfdata.warning = parseFloat(
                this.perfdata.warning,
            ).toString();
            this.perfdata.critical = parseFloat(
                this.perfdata.critical,
            ).toString();
