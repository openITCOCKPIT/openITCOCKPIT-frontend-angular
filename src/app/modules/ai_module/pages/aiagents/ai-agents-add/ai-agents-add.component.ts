...
@Component({
    ...
    templateUrl: './ai-agents-add.component.html',
    styleUrl: './ai-agents-add.component.css',
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class AiAgentsAddComponent implements OnInit, OnDestroy {
    ...
    public submit(): void {
        this.subscriptions.add(this.AiAgentsService.add(this.post).subscribe(result => {
            this.cdr.markForCheck();

            if (result.success) {
                const response = result.data as GenericIdResponse;
                const title = this.TranslocoService.translate('AI agent');
                const message = this.TranslocoService.translate('created successfully');
