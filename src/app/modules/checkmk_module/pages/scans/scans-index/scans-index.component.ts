...
export class ScansIndexComponent implements OnInit, OnDestroy {
    ...
    public loadScansIndex() {
        this.subscriptions.add(
            this.ScansService.getScansConfiguration(this.hostId).subscribe(
                (data) => {
    ...
    public executeDiscovery() {
        ...
        this.subscriptions.add(
            this.ScansService.runAjaxHealthList(
                this.hostId,
                this.post,
                ...
                if (healthResponse.satTaskId) {
                    ...
                    if (this.discoveryResult?.missing_template) {
                        this.missingTemplates = Object.values(
                            this.discoveryResult.missing_template,
                        );
                    }
                ...
                this.subscriptions.add(
                    this.ScansService.runAjaxProcessList(this.hostId).subscribe(
                        (processResponse) => {
                            this.cdr.markForCheck();
                            ...
                            if (processResponse.satTaskId) {
                                ...
                                this.satTaskIdProcess =
                                    processResponse.satTaskId;
                                this.satStatusProcessUpdateInterval =
                                    setInterval(() => {
                                ...
                                if (processResponse.MkProcesses) {
                                    Object.keys(
                                        ...
                                        const typescriptKey =
                                            key as keyof ScansProcessListDiscoveryResult;
                                ...
                                if (this.satStatusHealthUpdateInterval) {
                                    clearInterval(
                                        this.satStatusHealthUpdateInterval,
                                    );
                                }
                                if (this.satStatusProcessUpdateInterval) {
                                    clearInterval(
    ...
    public cancelSatRequest() {
        ...
        this.notyService.genericInfo(
            'Waiting for satellite result got cancelled',
        );
    }

    private checkSatelliteHealthStatus() {
        this.subscriptions.add(
            this.ScansService.getSatelliteHealthDiscoveryStatus(
                this.satTaskIdHealth,
            ).subscribe((result) => {
                if (result.pending) {
    ...
    private checkSatelliteProcessStatus() {
        this.subscriptions.add(
            this.ScansService.getSatelliteProcessDiscoveryStatus(
                this.satTaskIdProcess,
            ).subscribe((result) => {
                if (result.pending) {
                    // No result yet - keep waiting
                ...
                if (result.MkProcesses) {
                    ...
                    Object.keys(result.MkProcesses).forEach((key: string) => {
                        ...
                        if (
                            result.MkProcesses &&
                            result.MkProcesses[typescriptKey]
                        ) {
    ...
    public submit() {
        ...
        this.subscriptions.add(
            this.ScansService.saveCheckmkServices(this.hostId, data).subscribe(
                (result) => {
                    this.cdr.markForCheck();
                    if (result.success) {
                        const response = result.data as GenericIdResponse;
                        const title =
                            this.TranslocoService.translate('Services');
