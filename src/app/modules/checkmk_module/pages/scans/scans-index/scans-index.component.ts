...
export class ScansIndexComponent implements OnInit, OnDestroy {
    ...
    public loadScansIndex() {
        this.subscriptions.add(
            this.ScansService.getScansConfiguration(this.hostId).subscribe((data) => {
    ...
    public executeDiscovery() {
        ...
        this.subscriptions.add(
            this.ScansService.runAjaxHealthList(this.hostId, this.post).subscribe((healthResponse) => {
                this.cdr.markForCheck();
                ...
                this.subscriptions.add(
                    this.ScansService.runAjaxProcessList(this.hostId).subscribe((processResponse) => {
                        this.cdr.markForCheck();
    ...
    private checkSatelliteHealthStatus() {
        this.subscriptions.add(
            this.ScansService.getSatelliteHealthDiscoveryStatus(this.satTaskIdHealth).subscribe((result) => {
                if (result.pending) {
    ...
    private checkSatelliteProcessStatus() {
        this.subscriptions.add(
            this.ScansService.getSatelliteProcessDiscoveryStatus(this.satTaskIdProcess).subscribe((result) => {
                if (result.pending) {
                    // No result yet - keep waiting
                    return;
                }
    ...
    public submit() {
        ...
        this.subscriptions.add(
            this.ScansService.saveCheckmkServices(this.hostId, data).subscribe((result) => {
                this.cdr.markForCheck();
                if (result.success) {
