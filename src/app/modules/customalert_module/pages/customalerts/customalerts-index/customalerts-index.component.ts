...
export class CustomalertsIndexComponent
    implements OnInit, OnDestroy, IndexPage
{
    ...
    private readonly CustomAlertsService: CustomAlertsService =
        inject(CustomAlertsService);
    ...
    protected filter: CustomAlertsIndexFilter =
        getDefaultCustomAlertsIndexFilter();
    protected containers: SelectKeyValue[] = [];
    protected params: CustomAlertsIndexParams =
        getDefaultCustomAlertsIndexParams();
    ...
    protected selectedAutoRefresh: SelectKeyValue = {
        key: 0,
        value: 'Disabled',
    };
    ...
    private getSelectedItems(customAlert?: Customalert): Customalert[] {
        ...
        return this.SelectionServiceService.getSelectedItems().map(
            (item): Customalert => {
                return item;
            },
        );
    ...
    public onSelectedBookmark(filterstring: string) {
        ...
        if (filterstring && filterstring.length > 0) {
            ...
            let filter: CustomAlertsIndexFilter =
                getDefaultCustomAlertsIndexFilter();
            filter.Customalerts.message =
                bookmarkfilter.Customalerts?.message || '';
            filter.Customalerts.state = bookmarkfilter.Customalerts?.state || [
                true,
                true,
                false,
                false,
            ...
            filter.Hosts.container_id =
                bookmarkfilter.Hosts?.container_id.filter(
                    (containerId: number) => {
    ...
    private loadContainers(): void {
        this.subscriptions.add(
            this.CustomAlertsService.loadContainers().subscribe(
                (result: LoadContainersRoot) => {
    ...
    protected refresh(): void {
        ...
        this.subscriptions.add(
            this.CustomAlertsService.getIndex(
                this.params,
                this.filter,
                ...
                for (var i in this.result.customalerts) {
                    ...
                    if (
                        !this.groupedList.hasOwnProperty(
                            customAlert.service.host_id,
                        )
                    ) {
                    ...
                    this.groupedList[customAlert.service.host_id].push(
                        customAlert,
                    );
                }
