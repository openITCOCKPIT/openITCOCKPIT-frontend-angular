...
export class InstantreportsAddComponent implements OnInit, OnDestroy {
    ...
    private loadTimeperiods() {
        this.subscriptions.add(
            this.TimeperiodsService.loadTimeperiodsByContainerId(this.post.container_id).subscribe((result) => {
    ...
    private loadUsers() {
        this.subscriptions.add(
            this.UsersService.loadUsersByContainerId(this.post.container_id, this.post.users._ids).subscribe(
                (result) => {
                    this.users = result;
                    this.cdr.markForCheck();
                },
    ...
    public loadServices = (searchString: string) => {
        ...
        this.subscriptions.add(
            this.ServicesService.loadServicesByString(params).subscribe((result) => {
