...
export class BackupsRestoreComponent implements OnInit, OnDestroy {
    ...
    public loadBackups(): void {
        this.subscriptions.add(
            this.BackupsService.getAvailableBackups().subscribe(
                (result: BackupIndexResponse) => {
                    this.backupFiles = [];
    ...
    public startRestore(): void {
        this.subscriptions.add(
            this.BackupsService.restoreBackup(this.selectedBackup).subscribe(
                (result) => {
                    this.cdr.markForCheck();
                    if (result.success) {
                        const response =
                            result.data as StartRestoreBackupResponse;
        ...
        if (this.selectedBackup !== '') {
            this.backupFiles.forEach((backup) => {
                if (backup.key === this.selectedBackup) {
                    ...
                    window.open(
                        `/backups/downloadBackupFile?filename=${filename}`,
                        '_blank',
                    );
                    window.focus();
                }
