...
import { ServiceStatusOverviewWidgetConfig } from './service-status-overview-widget.interface';
...
@Component({
    ...
    templateUrl: './service-status-overview-widget.component.html',
    styleUrl: './service-status-overview-widget.component.css',
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class ServiceStatusOverviewWidgetComponent extends BaseWidgetComponent implements OnDestroy, AfterViewInit {
    protected flipped = signal<boolean>(false);
    public config?: ServiceStatusOverviewWidgetConfig;
    private readonly ServiceStatusOverviewWidgetService = inject(ServiceStatusOverviewWidgetService);
    ...
    public override load() {
        if (this.widget) {
            this.subscriptions.add(this.ServiceStatusOverviewWidgetService.getServiceStatusOverviewWidget(this.widget)
                .subscribe((result) => {
    ...
    public submit() {
        ...
        this.subscriptions.add(this.ServiceStatusOverviewWidgetService.saveWidgetConfig(this.widget.id, this.config)
            .subscribe({
                next: (result) => {
                    this.cdr.markForCheck();
