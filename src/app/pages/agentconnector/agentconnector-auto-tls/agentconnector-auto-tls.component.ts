...
export class AgentconnectorAutoTlsComponent implements OnInit, OnDestroy {
    ...
    public loadAutoTls(reExchangeAutoTLS: boolean = false) {
        ...
        this.subscriptions.add(
            this.AgentconnectorService.loadAutoTls(
                this.hostId,
                reExchangeAutoTLS,
            ).subscribe((data) => {
                this.cdr.markForCheck();
                ...
                if (data.satellite_task_id) {
                    ...
                    this.disableNextButton =
                        data.connection_test.status !== 'success';
                }
            }),
        );
    }
    ...
    private checkForSatelliteResponse() {
        if (this.satellite_task_id) {
            const sub = this.AgentconnectorService.loadSatelliteResponse(
                this.satellite_task_id,
            ).subscribe({
                next: (
                    response: AgentconnectorAutoTlsSatelliteTaskResponse,
                ) => {
                    // 200 ok
                    this.cdr.markForCheck();
                    ...
                    if (
                        ...
                        if (
                            ...
                            this.connection_test =
                                responseJson as AgentconnectorAutoTlsConnectionTest;

                            this.disableNextButton =
                                this.connection_test.status !== 'success';
                        }
                    }
