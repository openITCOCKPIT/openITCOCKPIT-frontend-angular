...
export class HostgroupsExtendedComponent implements OnInit, OnDestroy {
    ...
    protected hostgroupExtendedRoot: HostgroupExtendedRoot =
        {} as HostgroupExtendedRoot;
    ...
    public hostParams: HostgroupsExtendedParams =
        getDefaultHostgroupsExtendedParams();
    ...
    private loadHostgroupExtended(): void {
        this.subscriptions.add(
            this.HostgroupsService.loadHostgroupWithHostsById(
                this.hostgroupId,
                this.hostParams,
            ).subscribe((result: HostgroupExtendedRoot) => {
                this.hostgroupExtended = result.hostgroup;
                this.userFullname = result.username;
                this.hostgroupExtended.Hosts.forEach(
                    (host: HostGroupExtendedHost) => {
                        host.serviceParams =
                            getDefaultHostgroupsExtendedServiceListParams();
    ...
    public loadHostgroups = (searchString: string) => {
        ...
        this.subscriptions.add(
            this.HostgroupsService.loadHostgroupsByString(params).subscribe(
    ...
    private loadAdditionalInformation(): void {
        this.subscriptions.add(
            this.HostgroupsService.loadAdditionalInformation(
                this.hostgroupId,
            ).subscribe((result: HostgroupAdditionalInformation) => {
                this.AdditionalInformationExists =
                    result.AdditionalInformationExists;
                this.cdr.markForCheck();
            }),
        );
    }
    ...
    public toggleDowntimeModal() {
        this.selectedItems = this.hostgroupExtended.Hosts.map(
            (host): HostDowntimeItem => {
                return {
    ...
    public enableNotifications() {
        this.selectedItems = this.hostgroupExtended.Hosts.map(
            (host): HostEnableNotificationsItem => {
                return {
    ...
    private loadServicesList(element: HostGroupExtendedHost): void {
        ...
        if (element.Servicestatus.current_state.ok) {
            element.serviceParams['filter[Servicestatus.current_state][]'].push(
                'ok',
            );
        }
    ...
    protected onHostFilterChange(event: Event): void {
        ...
        if (this.filter.Hoststatus.current_state.unreachable) {
            this.hostParams['filter[Hoststatus.current_state][]'].push(
                'unreachable',
            );
        }
    ...
    public onServicePaginatorChange(
        change: PaginatorChangeEvent,
        host: HostGroupExtendedHost,
    ): void {
