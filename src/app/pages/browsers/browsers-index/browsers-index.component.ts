...
import {
    ContainerTypesEnum,
    ROOT_CONTAINER,
} from '../../changelogs/object-types.enum';
...
import {
    BrowsersContainer,
    BrowsersIndexResponse,
    StatuscountResponse,
} from '../browsers.interface';
...
export class BrowsersIndexComponent implements OnInit, OnDestroy, IndexPage {
    ...
    public ngOnInit(): void {
        ...
        this.route.queryParams.subscribe((params) => {
            ...
            this.subscriptions.add(
                this.BrowsersService.getIndex(this.containerId).subscribe(
                    (result) => {
                        ...
                        // Update the title.
                        let newTitle: string =
                            this.containers.breadcrumbs[
                                this.containers.breadcrumbs.length - 1
                            ].value;
                        this.TitleService.setTitle(
                            `${newTitle} | ` +
                                this.TranslocoService.translate('Browser'),
                        );
                    },
                ),
            );
    ...
    private loadStatusCounts(containerId: number) {
        this.subscriptions.add(
            this.BrowsersService.getStatusCountsByContainer([
                containerId,
            ]).subscribe((result) => {
    ...
    public getFilteredContainers(): BrowsersContainer[] {
        ...
        return this.containers.containers.filter((container: any) =>
            container.value.name
                .toLowerCase()
                .includes(this.containerFilter.toLowerCase()),
        );
    }
