...
export class ExportsIndexComponent implements OnInit, OnDestroy {
                            ...
                            if (data.exportFinished) {
                                ...
                                if (!data.exportSuccessfully) {
                                    ...
                                    this.notyService.genericError(
                                        this.TranslocoService.translate("Refresh of monitoring configuration failed."),
                                    );
                                    return;
                                }
            ...
            this.ExportsService.launchExport({
                ...
                if (result.success) {
                    this.notyService.genericInfo(
                        this.TranslocoService.translate("Refresh of monitoring configuration started successfully."),
                    );
                            ...
                            if (data.exportFinished) {
                                ...
                                if (!data.exportSuccessfully) {
                                    ...
                                    for (const task of data.tasks) {
                                        if (
                                            (task.task === "export_verify_new_configuration" &&
                                                task.finished === 1 &&
                                                task.successfully === 0) ||
                                            (task.task === "export_verify_new_prometheus_configuration" &&
                                                task.finished === 1 &&
                                                task.successfully === 0)
                                        ) {
    ...
    public saveSatelliteSelection() {
        ...
        if (post.length > 0) {
            this.subscriptions.add(
                this.ExportsService.saveSatelliteSelection(post).subscribe((result) => {
                    this.cdr.markForCheck();
                    if (result.success) {
