...
export class ConfigurationFilesEditComponent implements OnInit, OnDestroy {
    ...
    public loadConfigurationFile(): void {
        if (this.dbKey) {
            this.subscriptions.add(
                this.ConfigurationFilesService.getConfigFileForEdit(
                    this.dbKey,
                ).subscribe((data) => {
    ...
    public restore() {
        ...
        if (this.dbKey && this.ConfigFile) {
            this.ConfigurationFilesService.restoreDefault(
                this.dbKey,
                this.ConfigFile.moduleUrl,
            ).subscribe((response) => {
                this.cdr.markForCheck();
                ...
                if (response.success) {
                    this.notyService.genericSuccess(
                        this.TranslocoService.translate(
                            'Config successfully restored to default',
                        ),
                    );
                    this.hideModal();
