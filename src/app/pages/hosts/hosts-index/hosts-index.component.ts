...
export class HostsIndexComponent implements OnInit, OnDestroy, IndexPage {
    ...
    protected loadHostgroups = (search: string) => {
        this.subscriptions.add(
            ...
            } as HostgroupsLoadHostgroupsByStringParams).subscribe(
                (data: SelectKeyValue[]) => {
    ...
    public loadHosts() {
        ...
        if (
            this.acknowledgementsFilter.acknowledged !==
            this.acknowledgementsFilter.not_acknowledged
        ) {
            hasBeenAcknowledged = String(
                this.acknowledgementsFilter.acknowledged === true,
            );
        }
        if (
            this.downtimeFilter.in_downtime !==
            this.downtimeFilter.not_in_downtime
        ) {
            inDowntime = String(this.downtimeFilter.in_downtime === true);
        }
        if (
            this.notificationsFilter.enabled !==
            this.notificationsFilter.not_enabled
        ) {
        ...
        if (this.route.snapshot.queryParams.hasOwnProperty('filter.Hosts.id')) {
            this.filter['Hosts.id'] =
                this.route.snapshot.queryParams['filter.Hosts.id'];
        }
        this.filter['Hoststatus.problem_has_been_acknowledged'] =
            hasBeenAcknowledged;
        ...
        this.filter['Hoststatus.current_state'] = getHostCurrentStateForApi(
            this.currentStateFilter,
        );

        this.subscriptions.add(
            this.HostsService.getIndex(this.params, this.filter).subscribe(
                (result) => {
                    this.hosts = result;
    ...
    public linkFor(format: 'pdf' | 'csv') {
        ...
        let urlParams = {
            ...
            'filter[Hoststatus.current_state][]': getHostCurrentStateForApi(
                this.currentStateFilter,
            ),
    ...
    public resetChecktime() {
        this.selectedItems = [];
        let selectedHostIds: number[] =
            this.SelectionServiceService.getSelectedItems().map(
                (item) => item.Host.id,
            );

        this.hosts?.all_hosts.forEach((item) => {
            if (
                typeof item.Host.id === 'undefined' ||
                typeof item.Host.uuid === 'undefined'
            ) {
                return;
            }
    ...
    public onSelectedBookmark(filterstring: string) {
        ...
        if (filterstring && filterstring.length > 0) {
            ...
            this.filter['Hosts.name_regex'] =
                bookmarkfilter['Hosts.name_regex'] || false;
            this.filter['Hosts.address'] =
                bookmarkfilter['Hosts.address'] || '';
            this.filter['Hosts.address_regex'] =
                bookmarkfilter['Hosts.address_regex'] || false;
            this.filter['hostdescription'] =
                bookmarkfilter['hostdescription'] || '';
            this.filter['Hosts.host_type'] =
                bookmarkfilter['Hosts.host_type'] || [];
            this.filter['Hosts.keywords'] =
                bookmarkfilter['Hosts.keywords'] || [];
            this.filter['Hosts.not_keywords'] =
                bookmarkfilter['Hosts.not_keywords'] || [];
            this.filter['Hoststatus.output'] =
                bookmarkfilter['Hoststatus.output'] || '';
            this.convert2currentStateFilter(
                bookmarkfilter['Hoststatus.current_state'] || [],
                'currentStateFilter',
            );
            if (
                bookmarkfilter['Hoststatus.problem_has_been_acknowledged'] ===
                'true'
            ) {
                this.acknowledgementsFilter.acknowledged = true;
            }
            ...
            if (
                bookmarkfilter['Hoststatus.scheduled_downtime_depth'] === 'true'
            ) {
                this.downtimeFilter.in_downtime = true;
            }
            if (
                bookmarkfilter['Hoststatus.scheduled_downtime_depth'] ===
                'false'
            ) {
                this.downtimeFilter.not_in_downtime = true;
            }
            ...
            if (
                bookmarkfilter['Hoststatus.notifications_enabled'] === 'false'
            ) {
            ...
            this.convert2currentStateFilter(
                bookmarkfilter['hostpriority'] || [],
                'priorityFilter',
            );
            this.filter['Hosts.satellite_id'] =
                bookmarkfilter['Hosts.satellite_id'] || [];
            this.filter['Hostgroups.id'] =
                bookmarkfilter['Hostgroups.id'] || [];
            this.loadHosts();
        }
    }

    protected convert2currentStateFilter(
        state_array: string[],
        filter: string,
    ): void {
    ...
    public loadColumns() {
        if (this.LocalStorageService.hasItem(this.columnsTableKey, 'true')) {
            this.fields = JSON.parse(
                String(this.LocalStorageService.getItem(this.columnsTableKey)),
            );
        }
    }
