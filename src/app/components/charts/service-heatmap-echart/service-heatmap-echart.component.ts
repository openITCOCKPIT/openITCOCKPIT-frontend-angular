...
import { SummaryStateServicesTag } from '../../../pages/services/summary_state.interface';

echarts.use([CustomChart, ScatterChart, GridComponent, TooltipComponent, VisualMapComponent, CanvasRenderer]);

@Component({
    selector: "oitc-service-heatmap-echart",
    imports: [
        NgxEchartsDirective
    ],
    providers: [provideEchartsCore({echarts})],
    templateUrl: "./service-heatmap-echart.component.html",
    styleUrl: "./service-heatmap-echart.component.css",
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ServiceHeatmapEchartComponent implements OnDestroy, AfterViewInit {
    ...
    public chartData = input.required<{ [key: string]: SummaryStateServicesTag }>();
        ...
        this.echartsInstance.on('datarangeselected', (params: any) => {
            ...
            Object.keys(selected).forEach((key) => {
                const piece = Number.isNaN(Number(key))
                    ? ServiceHeatmapEchartComponent.PIECES
                        .find(p => this.TranslocoService.translate(p.label) === key)
                    : ServiceHeatmapEchartComponent.PIECES[Number(key)];
    ...
    private renderHeatmap() {
            ...
            .map(([name, data]) => {
                ...
                return {
                    ...
                    details: ServiceHeatmapEchartComponent.buildTooltipTable([
                        ...
                        {
                            label: this.TranslocoService.translate('Total services'),
                            value: total,
                            className: ''
                        }
                    ])
        ...
        const columnsCount = Math.min(
            ServiceHeatmapEchartComponent.MAX_COLUMNS,
        ...
        this.chartHeightPx = Math.max(
            160,
            (rowsCount * ServiceHeatmapEchartComponent.ROW_HEIGHT) + ServiceHeatmapEchartComponent.LEGEND_HEIGHT
        );
        ...
        const colorForState = (state: number) => {
            if (state === 0) return gradientOk;
            if (state === 1) return gradientWarning;
            if (state === 2) return gradientCritical;
            if (state === 3) return gradientUnknown;
        ...
        this.chartOption = {
            ...
            grid: {
                left: 8,
                right: 8,
                top: ServiceHeatmapEchartComponent.LEGEND_HEIGHT,
                bottom: 8
            },
            ...
            visualMap: {
                ...
                selected: ServiceHeatmapEchartComponent.PIECES.reduce((acc, piece, index) => {
